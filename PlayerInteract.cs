...
public class PlayerInteract : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetButtonDown("Submit") && currentObject != tripWireArray[0] && currentObject != null && tripWirePlanted == true ) //somehow use a WHILE loop for while the player is planting a tripwire?
        {
            ...
            if (Vector3.Distance(tripWireArray[0].transform.position, tripDestinationPos) < 7f)
            {
                ...
                //Quaternion rotationWire = Quaternion.LookRotation(currentObject.transform.position);

                //float rotationy = Vector3.Angle(currentObject.transform.position, tripWireArray[0].transform.position);

                //Quaternion wireRotation= Quaternion.LookRotation(tripWireArray[0].transform.position);
    ...
    public void ButtonClicked(string trapType) //called from the SENDMESSAGE in "PlayerInventory" script and a string is sent and stored in "trapType"
    {
        ...
        if (trapType == "Drone")
        {
            Instantiate(drone, transform.position, drone.transform.rotation, gameObject.transform); //instantiates the drone
